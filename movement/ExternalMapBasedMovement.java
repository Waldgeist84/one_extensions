/* 
 * Copyright 2014 TU Ilmenau, Communication Networks Group
 ...
 */
public class ExternalMapBasedMovement extends MapBasedMovement implements SwitchableMovement {
	...
	private static Map<String, ExternalMapBasedMovement> idMapping;
	 ...
	 */
	public ExternalMapBasedMovement(Settings settings) {
		super(settings);

		if (idMapping == null) {
			idMapping = new HashMap<String, ExternalMapBasedMovement>();
			inputFileName = settings.getSetting(MOVEMENT_FILE_S);
			reader = new ExternalMovementReader(inputFileName);
			
			//initLocations = reader.readNextMovements();
			initLocations = readMovements(true, getMap());
			initTime = reader.getLastTimeStamp();
			samplingInterval = -1;
			lastPreloadTime = -1;
			
			if (settings.contains(NROF_PRELOAD_S)) {
				nrofPreload = settings.getInt(NROF_PRELOAD_S);
				if (nrofPreload <= 0) {
					nrofPreload = 1;
				}
			}
	 ...
	 */	
	public ExternalMapBasedMovement(Settings settings, SimMap newMap, int nrofMaps) {
	...
	private static List<Tuple<String, Coord>> readMovements(boolean translate, SimMap map){
		List<Tuple<String, Coord>> input = reader.readNextMovements();
		...
		if(translate){
			...
			for (Tuple<String, Coord> t : input) {	
				Coord c = t.getValue();
	 ...
	 */
	private ExternalMapBasedMovement(MapBasedMovement mbm) {
		super(mbm);
	 ...
	 * needed.
	 */
	private static void checkPathNeed(SimMap map) {
		if (samplingInterval == -1) { // first preload
			lastPreloadTime = readMorePaths(map);
		}
	...
	public Path getPath() {
		...
		checkPathNeed(getMap()); // check if we should preload more paths		
	...
	@Override
	public ExternalMapBasedMovement replicate() {
		return new ExternalMapBasedMovement(this);
	}
	 ...
	 * were read.
	 */
	private static double readMorePaths(SimMap map) {
//		List<Tuple<String, Coord>> list = reader.readNextMovements();
		List<Tuple<String, Coord>> list = readMovements(true, map);
		...
		for (Tuple<String, Coord> t : list) {
			ExternalMapBasedMovement em = idMapping.get(t.getKey());
